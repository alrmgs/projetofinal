$(function () {
    ...
    $("#btn_add_feeling").click(function () {
        clearErrors();
        $("#form_feeling")[0].reset();
        $("#feeling_img_path").attr("src", "");
        $("#modal_feeling").modal();
    });
    ...
    $("#btn_upload_feeling_img").change(function () {
        uploadImg($(this), $("#feeling_img_path"), $("#feeling_img"));
    });
    ...
    $("#form_feeling").submit(function () {

        $.ajax({
            type: "POST",
            url: BASE_URL + "restrict/ajax_save_feeling",
            dataType: "json",
            data: $(this).serialize(),
            beforeSend: function () {
                clearErrors();
                $("#btn_save_feeling").siblings(".help-block").html(loadingImg("Verificando..."));
            },
            success: function (response) {
                clearErrors();
                if (response["status"]) {
    ...
    function active_btn_feeling() {

        $(".btn-edit-feeling").click(function () {
            $.ajax({
                type: "POST",
                url: BASE_URL + "restrict/ajax_get_feeling_data",
                dataType: "json",
                data: {"feeling_id": $(this).attr("feeling_id")},
                success: function (response) {
                    clearErrors();
                    $("#form_feeling")[0].reset();
                    $.each(response["input"], function (id, value) {
                    ...
                    $("#feeling_img_path").attr("src", response["img"]["feeling_img_path"]);
                    $("#modal_feeling").modal();
                }
            })
        });

        $(".btn-del-feeling").click(function () {

            feeling_id = $(this);
            swal({
                title: "Atenção!",
                text: "Deseja deletar essa postagem?",
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#d9534f",
                confirmButtonText: "Sim",
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: BASE_URL + "restrict/ajax_delete_feeling_data",
                        dataType: "json",
    ...
    var dt_feeling = $("#dt_feelings").DataTable({
        "oLanguage": DATATABLE_PTBR,
        "autoWidth": false,
        "processing": true,
        "serverSide": true,
        "ajax": {
            "url": BASE_URL + "restrict/ajax_list_feeling",
            "type": "POST",
        },
        "columnDefs": [
            {targets: "no-sort", orderable: false},
